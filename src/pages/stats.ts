// ============================================================
// 고수치과의원(gosudc.kr) 관리자 통합 통계 — 중앙 대시보드(PF Web Engine) 연동
// 토큰은 서버사이드 API 호출에만 사용
// ============================================================
import { raw } from 'hono/html'

const STATS_API_URL = 'https://pf-dashboard-2nt.pages.dev/api/stats/gosudc.kr'



export async function fetchSiteStats(token?: string): Promise<any | null> {
  if (!token) return null
  try {
    const res = await fetch(STATS_API_URL, { headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(8000) })
...
const CLARITY_URL = 'https://clarity.microsoft.com/projects/view/yenfpw6ija/dashboard'
...
const STATS_CSS = `
.stx-root{--brand-2:var(--sky-deep);--ink-3:var(--ink-mute);--bg:#f4f7f9;--r:4px;font-family:var(--sans)}
.stx-root .stx-head{display:flex;align-items:flex-end;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-bottom:22px}
.stx-root .stx-head h1{margin-bottom:4px}
.stx-root .stx-head .sub{color:var(--ink-mute);font-size:0.86rem}
...
.stx-expect-lg{padding:38px 30px;text-align:center;background:linear-gradient(165deg,var(--sky-mist) 0%,#fff 45%);border-color:var(--sky-line)}
.stx-expect-icon{width:52px;height:52px;border-radius:14px;background:var(--sky-mist);color:var(--brand);display:flex;align-items:center;justify-content:center;font-size:1.3rem;margin:0 auto 16px}
...
.stx-tl-dot{width:28px;height:28px;border-radius:50%;background:var(--sky-mist);border:1px solid var(--brand);color:var(--brand);font-weight:800;font-size:0.78rem;display:flex;align-items:center;justify-content:center;margin:0 auto 7px}
...
.stx-tl-line{flex:0 0 20px;height:1px;background:var(--sky-line);align-self:center;margin-top:-22px}
...
.stx-clarity-link{font-size:0.72rem;color:var(--brand);font-weight:700;margin-left:10px;border:1px solid var(--sky-line);padding:3px 10px;border-radius:99px;text-decoration:none;transition:background .2s}
.stx-clarity-link:hover{background:var(--sky-mist)}
...
.stx-insight{background:linear-gradient(165deg,var(--sky-mist) 0%,#fff 55%);border:1px solid var(--sky-line);border-radius:var(--r);padding:20px 24px;margin:24px 0}
...
export function AdminStats(d: any) {
  ...
  let inner = `<div class="stx-root"><style>${STATS_CSS}</style>`
  inner += `<div class="stx-head"><div><h1><i class="fa-solid fa-chart-line" style="color:var(--brand)"></i> 통합 통계</h1>
    <div class="sub">고수치과의원(gosudc.kr) 검색·방문·AI 유입 성과 (중앙 대시보드 연동)</div></div>
  ...
  if (g) {
    ...
    inner += `<div class="stx-spark"><div class="stx-spark-title">일별 검색 클릭</div>${sparkline((g.dailyClicks ?? []).map((x: any) => Number(x.clicks) || 0), '#4f9cbe')}</div>`
  } else {
  ...
  if (a) {
    ...
    inner += `<div class="stx-spark"><div class="stx-spark-title">일별 사용자</div>${sparkline((a.dailyUsers ?? []).map((x: any) => Number(x.users) || 0), '#2e353b')}</div>`
  } else {
