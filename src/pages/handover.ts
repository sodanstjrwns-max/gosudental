...
// 납품 안내서 — 도담 안내서와 같은 구조. 분석 스크립트·색인 제외, 관리자 비밀번호는 운영 도메인에서만 env 값을 표시.
export function handoverPage(c: Context<any>) {
  const url = new URL(c.req.url)
  const deliveryOrigin = url.origin === 'https://gosudc.kr' || ['localhost', '127.0.0.1'].includes(url.hostname)
...
<link rel="canonical" href="https://gosudc.kr/handover"><link rel="icon" href="/favicon.ico">
...
<link rel="stylesheet" href="/static/handover.css?v=2">
  ...
  <div class="document-body">
    ...
    <section id="delivery-highlights">
      ...
      <p>관리자에서 치료사례와 칼럼을 올리면 <strong>즉시 검색엔진 색인 신호(IndexNow)</strong>가 전송되고 사이트맵에 반영됩니다. 원장님이 올리는 만큼 검색 입구가 늘어납니다.</p>
    ...
    <section id="delivery-structure">
      ...
      <p class="sec-desc">동네 치과 홈페이지가 보통 10~20페이지일 때, 고수치과는 148개 페이지로 시작합니다.</p>
      <table class="spec-table"><caption class="sr-only">고수치과 홈페이지 구성과 기능</caption><thead><tr><th scope="col">항목</th><th scope="col">내용</th></tr></thead><tbody>
        <tr><th scope="row">전체 공개 안내</th><td><strong>사이트맵 URL 148개</strong> — 병원·의료진·진료·용어사전·지역 안내를 연결하는 구조입니다.</td></tr>
        <tr><th scope="row">치과 용어사전</th><td><strong>104개 용어 페이지</strong> — 어려운 용어를 쉬운 말로 설명하고 관련 진료로 연결합니다.</td></tr>
        <tr><th scope="row">지역 × 진료</th><td><strong>18개 안내 페이지</strong> — 내포신도시·예산·삽교·홍성 등 주변 지역과 진료를 연결하는 로컬 SEO 페이지입니다.</td></tr>
        <tr><th scope="row">진료 상세 안내</th><td><strong>8개 진료 페이지</strong> — 임플란트·치아교정·심미보철·라미네이트 등 증상–원인–치료–FAQ까지 담았습니다.</td></tr>
        <tr><th scope="row">의료진 소개</th><td><strong>4명 개별 페이지</strong> — 조원익·김경환·이민우 원장 등 경력과 진료 철학을 소개합니다.</td></tr>
        <tr><th scope="row">치료사례·칼럼</th><td>관리자에서 올리는 즉시 공개·색인 신호 전송. 개수 제한 없이 계속 쌓입니다.</td></tr>
        ...
        <tr><th scope="row">모바일·속도</th><td>모바일 하단 고정 버튼(전화·예약·길찾기), WebP 이미지·지연 로딩, PWA 설치 지원.</td></tr>
    ...
    <section id="delivery-operations">
      ...
      <h3>들어가는 방법</h3><ul><li>브라우저에서 <a href="https://gosudc.kr/admin/login"><strong>https://gosudc.kr/admin/login</strong></a>에 접속합니다.</li><li>아래 관리자 비밀번호를 입력하면 <strong>바로 관리 화면</strong>이 열립니다.</li></ul>
      ...
      <aside class="warn"><p class="title">로그인 정보가 포함된 안내서입니다.</p><p><strong>이 주소는 링크를 아는 사람이 열 수 있으므로</strong> 문서·링크를 외부에 공유하지 마세요. 비밀번호 변경이 필요하면 언제든 말씀해 주세요.</p></aside>
      <h3>관리자에서 할 수 있는 것</h3><ul>
        ...
        <li><strong>치료사례</strong> — 전후 사진과 설명을 올립니다. 저장 즉시 공개·색인 신호 전송.</li>
        <li><strong>원장 칼럼</strong> — 제목·본문·메타 설명·키워드를 입력해 발행합니다.</li>
        <li><strong>공지사항</strong> — 휴진·이벤트를 등록하고 홈 팝업으로 띄웁니다.</li>
      ...
      <aside class="callout"><p class="title">칼럼 쓰는 법</p><p><a href="/admin/posts">관리자 → 원장 칼럼 → 새 칼럼 작성</a> → 제목·본문과 함께 <strong>메타 설명(120~160자)·키워드</strong>를 꼭 채워 주세요. 검색 노출의 핵심입니다.</p></aside>
      ...
      <p class="note"><b>사진·저장 안내</b><br>JPG·PNG·WebP, 파일당 8MB를 지원합니다. HEIC는 변환해 주세요. 저장 완료 안내를 확인한 뒤 화면을 닫아 주세요.</p>
    </section>
    ...
    <section id="delivery-indexing">
      ...
      <aside class="warn"><p class="title">수집·색인·노출은 서로 다른 단계입니다.</p><p>홈페이지와 사이트맵을 준비했다고 모든 페이지가 바로 검색에 뜨지 않습니다. 검색엔진이 148개 페이지를 충분히 색인하고 순위를 올리기까지 <strong>대략 2~6개월</strong>이 걸립니다. 모든 새 홈페이지가 거치는 과정입니다.</p></aside>
    ...
    <section id="delivery-limits">
      ...
      <aside class="note"><p class="title">딱 하나, 제게 없는 실제 사진입니다.</p><p>없는 진료 사진이나 병원 사진을 대신 만들어드릴 수는 없습니다. 좋은 사진이나 영상이 생기면 보내 주세요. 받는 즉시 반영합니다.</p></aside>
