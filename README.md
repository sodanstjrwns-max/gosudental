...
- `/llms-full.txt`: 103개 전체 본문·FAQ·상담 질문·참고 URL 수록. HTTP 200 및 정적 원고와 전문 일치 확인. FAQ JSON-LD는 실제 화면과 같은 데이터 사용. 기존 canonical/H1/DefinedTerm 유지.
- 원고 원본은 `src/data/encyclopedia-content.ts`. D1/R2 변경 없이 정적 빌드에 포함하며 런타임 AI 호출·API 키 노출 없음. FDA/NHS/NIDCR 및 학술 자료 15개를 참고자료로 연결.
